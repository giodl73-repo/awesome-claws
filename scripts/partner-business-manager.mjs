...
function hasAuthorizedActor(clause, match, principalActors, right) {
  const prefix = clause.slice(0, match.index);
  const suffix = clause.slice(match.index + match[0].length);
  return principalActors.some(({ names, rights }) => {
    if (!rights.includes(right)) return false;
    return names.some((name) => {
      const escaped = name.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&");
      const active = new RegExp(
        `(?<![\\p{L}\\p{N}])${escaped}(?![\\p{L}\\p{N}])(?:\\s+(?:has|have|had|did|does|is|are|was|were|will|already|formally|explicitly))*\\s*$`,
        "u",
      ).test(prefix);
      const passive = new RegExp(
        `^\\s+by\\s+(?:the\\s+)?${escaped}(?![\\p{L}\\p{N}])`,
        "u",
      ).test(suffix);
...
function hasAffirmativeProhibitedAuthority(text, principalActors) {
  return text
    .split(/(?<=[.!?;])\s+|\s+\bbut\b\s+/iu)
    .some((clause) =>
      PROHIBITED_AUTHORITY_PATTERNS.some(({ pattern, right }) => {
        const matches = clause.matchAll(new RegExp(pattern.source, `${pattern.flags}g`));
        return [...matches].some((match) => {
          const normalizedClause = clause.toLocaleLowerCase("en-US");
          const hasAuthorizedExternalActor = hasAuthorizedActor(
            normalizedClause,
            match,
            principalActors,
            right,
          );
          if (hasAuthorizedExternalActor) return false;
          const prefix = clause.slice(0, match.index);
          const suffix = clause.slice(match.index + match[0].length);
          return !(
...
export function partnerBusinessPlanFindings(
  ...
  const principalActors = principalRows
    .filter(
      (principal) =>
    ...
    .map((principal) => {
      const name = principal.name.trim().toLocaleLowerCase("en-US");
      const roleNames = name.match(
        /\b(?:finance|program|sales|legal|risk|partner|vendor)\s+(?:owner|lead|principal|team)\b/gu,
      ) ?? [];
      return {
        names: [name, ...roleNames],
        rights: strings(principal.decisionRights),
      };
    });
  ...
  for (const [path, text] of narrativeEntries(value)) {
    if (hasAffirmativeProhibitedAuthority(text, principalActors)) {
      add(
        "prohibited_authority_narrative",
        path,
        "User-visible text must not affirm that the agent or artifact exercised reserved authority; use explicit negation, pending, or owner-reserved language.",
      );
    }
  }
