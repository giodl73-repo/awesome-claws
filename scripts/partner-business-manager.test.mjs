...
test("contradictory commitment approval assertions require reciprocal conflict accounting", () => {
  const addDenial = (value) => {
    const denial = structuredClone(
      value.evidence.find((row) => row.id === "evidence-commitment-vendor"),
    );
    Object.assign(denial, {
      id: "evidence-commitment-denied",
      assertion: "approval-denied",
      sourceRef: "controlled://jbp/vendor-commitment-denial-r4",
      observedAt: "2026-09-10T18:13:00Z",
    });
    value.evidence.push(denial);
    value.revisionDelta
      .find((row) => row.domain === "commitments")
      .evidenceRefs.push(denial.id);
    resealPartnerBusinessPlan(value);
    bindCurrentEvidence(value);
    return denial;
  };
  ...
  addDenial(unaccounted);
  hasCode(unaccounted, "unaccounted_evidence_conflict");

  const accounted = clone();
  const denial = addDenial(accounted);
  const approvalEvidence = accounted.evidence.filter(
    (row) =>
      row.subjectRef === denial.subjectRef &&
      ["approval-granted", "approval-denied"].includes(row.assertion),
  );
  for (const row of approvalEvidence) {
    row.conflictRefs.push("conflict-commitment-approval");
  }
  accounted.evidenceGovernance.conflicts.push({
    id: "conflict-commitment-approval",
    domain: "commitments",
    scope: "current-revision",
    evidenceRefs: approvalEvidence.map((row) => row.id),
    state: "unresolved",
    disposition: "Commitment owners must reconcile the contradictory approval records.",
    dispositionOwnerRef: "principal-partner-lead",
    dispositionEvidenceRefs: [],
  });
  resealPartnerBusinessPlan(accounted);
  bindCurrentEvidence(accounted);
  assert.equal(codes(accounted).has("unaccounted_evidence_conflict"), false);
});
...
test("rejects affirmative prohibited-authority narratives and allows negated or pending controls", () => {
  for (const [mutate, path] of [
    ...
    [
      (value) =>
        value.risks[0].response =
          "Partner incentives finance owner approved incentive.",
      "risks[0].response",
    ],
  ]) {
    const value = clone();
  ...
  hasCode(ownerHistory, "prohibited_authority_narrative");
  ...
  hasCode(ownerHistoryWithDocumentation, "prohibited_authority_narrative");
  ...
  hasCode(exactActor, "prohibited_authority_narrative");
