...
test("inventory derives the current portfolio oracle surface", async () => {
  ...
  assert.equal(inventory.summary.clawCount, 112);
  assert.equal(
  ...
  assert.equal(inventory.summary.packagedSchemaCount, 112);
  assert.equal(
...
test("schema portfolio covers every Claw and active constraint family", async () => {
  ...
  assert.equal(run.coverage.status, "passed");
  assert.equal(run.coverage.clawCount, 112);
  assert.equal(run.coverage.counts["control-failed"], 0);
  assert.equal(run.coverage.counts.survived, 0);
  assert.equal(run.coverage.counts["unsupported-oracle"], 0);
  assert.equal(run.coverage.counts["oracle-error"], 0);
  assert.equal(run.coverage.safety.caseCount, 672);
  assert.equal(run.coverage.safety.blockingCount, 0);
  assert.equal(run.coverage.schema.clawCount, 112);
...
test("semantic portfolio covers every registered owner-defined validator", async () => {
  ...
  assert.equal(run.coverage.safety.caseCount, 672);
  assert.equal(run.coverage.safety.blockingCount, 0);
  assert.equal(run.coverage.semantics.applicableClawCount, 112);
  assert.equal(run.coverage.semantics.caseCount, 304);
  assert.equal(run.coverage.semantics.killedCount, 304);
  assert.equal(run.coverage.semantics.findingCodeCount, 363);
  assert.equal(Object.keys(run.coverage.semantics.perClaw).length, 112);
...
test("lifecycle portfolio classifies faults and removes every capability adapter", async () => {
  ...
  assert.equal(run.coverage.safety.caseCount, 672);
  assert.equal(run.coverage.safety.blockingCount, 0);
  assert.equal(run.coverage.lifecycle.clawCount, 112);
  assert.equal(run.coverage.lifecycle.caseCount, 560);
  assert.equal(run.coverage.lifecycle.killedCount, 560);
  assert.equal(
  ...
  assert.equal(run.coverage.capabilities.applicableClawCount, 52);
  assert.equal(run.coverage.capabilities.caseCount, 75);
  assert.equal(run.coverage.capabilities.killedCount, 75);
