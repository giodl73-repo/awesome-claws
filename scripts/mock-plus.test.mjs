...
test("inventory derives the current portfolio oracle surface", async () => {
  ...
  assert.equal(inventory.summary.clawCount, 113);
  assert.equal(
  ...
  assert.equal(inventory.summary.packagedSchemaCount, 113);
  assert.equal(
...
test("schema portfolio covers every Claw and active constraint family", async () => {
  ...
  assert.equal(run.coverage.status, "passed");
  assert.equal(run.coverage.clawCount, 113);
  assert.equal(run.coverage.counts["control-failed"], 0);
  assert.equal(run.coverage.counts.survived, 0);
  assert.equal(run.coverage.counts["unsupported-oracle"], 0);
  assert.equal(run.coverage.counts["oracle-error"], 0);
  assert.equal(run.coverage.safety.caseCount, 678);
  assert.equal(run.coverage.safety.blockingCount, 0);
  assert.equal(run.coverage.schema.clawCount, 113);
...
test("semantic portfolio covers every registered owner-defined validator", async () => {
  ...
  assert.equal(run.coverage.safety.caseCount, 678);
  assert.equal(run.coverage.safety.blockingCount, 0);
  assert.equal(run.coverage.semantics.applicableClawCount, 113);
  assert.equal(run.coverage.semantics.caseCount, 307);
  assert.equal(run.coverage.semantics.killedCount, 307);
  assert.equal(run.coverage.semantics.findingCodeCount, 368);
  assert.equal(Object.keys(run.coverage.semantics.perClaw).length, 113);
...
test("lifecycle portfolio classifies faults and removes every capability adapter", async () => {
  ...
  assert.equal(run.coverage.safety.caseCount, 678);
  assert.equal(run.coverage.safety.blockingCount, 0);
  assert.equal(run.coverage.lifecycle.clawCount, 113);
  assert.equal(run.coverage.lifecycle.caseCount, 565);
  assert.equal(run.coverage.lifecycle.killedCount, 565);
  assert.equal(
  ...
  assert.equal(run.coverage.capabilities.applicableClawCount, 53);
  assert.equal(run.coverage.capabilities.caseCount, 76);
  assert.equal(run.coverage.capabilities.killedCount, 76);
