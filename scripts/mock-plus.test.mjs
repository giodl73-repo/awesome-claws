...
test("inventory derives the current portfolio oracle surface", async () => {
  ...
  assert.equal(inventory.summary.clawCount, 130);
  assert.equal(
  ...
  assert.equal(inventory.summary.packagedSchemaCount, 139);
  assert.equal(
...
test("schema portfolio covers every Claw and active constraint family", async () => {
  ...
  assert.equal(run.coverage.status, "passed");
  assert.equal(run.coverage.clawCount, 130);
  assert.equal(run.coverage.counts["control-failed"], 0);
  assert.equal(run.coverage.counts.survived, 0);
  assert.equal(run.coverage.counts["unsupported-oracle"], 0);
  assert.equal(run.coverage.counts["oracle-error"], 0);
  assert.equal(run.coverage.safety.caseCount, 780);
  assert.equal(run.coverage.safety.blockingCount, 0);
  assert.equal(run.coverage.schema.clawCount, 130);
...
test("semantic portfolio covers every registered owner-defined validator", async () => {
  ...
  assert.equal(run.coverage.safety.caseCount, 780);
  assert.equal(run.coverage.safety.blockingCount, 0);
  assert.equal(run.coverage.semantics.applicableClawCount, 130);
  assert.equal(run.coverage.semantics.caseCount, 358);
  assert.equal(run.coverage.semantics.killedCount, 358);
  assert.equal(run.coverage.semantics.findingCodeCount, 482);
  assert.equal(Object.keys(run.coverage.semantics.perClaw).length, 130);
...
test("lifecycle portfolio classifies faults and removes every capability adapter", async () => {
  ...
  assert.equal(run.coverage.safety.caseCount, 780);
  assert.equal(run.coverage.safety.blockingCount, 0);
  assert.equal(run.coverage.lifecycle.clawCount, 130);
  assert.equal(run.coverage.lifecycle.caseCount, 650);
  assert.equal(run.coverage.lifecycle.killedCount, 650);
  assert.equal(
