...
test("inventory derives the current portfolio oracle surface", async () => {
  ...
  assert.equal(inventory.summary.clawCount, 133);
  assert.equal(
  ...
  assert.equal(inventory.summary.packagedSchemaCount, 142);
  assert.equal(
...
test("schema portfolio covers every Claw and active constraint family", async () => {
  ...
  assert.equal(run.coverage.status, "passed");
  assert.equal(run.coverage.clawCount, 133);
  assert.equal(run.coverage.counts["control-failed"], 0);
  assert.equal(run.coverage.counts.survived, 0);
  assert.equal(run.coverage.counts["unsupported-oracle"], 0);
  assert.equal(run.coverage.counts["oracle-error"], 0);
  assert.equal(run.coverage.safety.caseCount, 798);
  assert.equal(run.coverage.safety.blockingCount, 0);
  assert.equal(run.coverage.schema.clawCount, 133);
...
test("semantic portfolio covers every registered owner-defined validator", async () => {
  ...
  assert.equal(run.coverage.safety.caseCount, 798);
  assert.equal(run.coverage.safety.blockingCount, 0);
  assert.equal(run.coverage.semantics.applicableClawCount, 133);
  assert.equal(run.coverage.semantics.caseCount, 367);
  assert.equal(run.coverage.semantics.killedCount, 367);
  assert.equal(run.coverage.semantics.findingCodeCount, 496);
  assert.equal(Object.keys(run.coverage.semantics.perClaw).length, 133);
...
test("lifecycle portfolio classifies faults and removes every capability adapter", async () => {
  ...
  assert.equal(run.coverage.safety.caseCount, 798);
  assert.equal(run.coverage.safety.blockingCount, 0);
  assert.equal(run.coverage.lifecycle.clawCount, 133);
  assert.equal(run.coverage.lifecycle.caseCount, 665);
  assert.equal(run.coverage.lifecycle.killedCount, 665);
  assert.equal(
