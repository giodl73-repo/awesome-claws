...
test("inventory derives the current portfolio oracle surface", async () => {
  ...
  assert.equal(inventory.summary.clawCount, 131);
  assert.equal(
  ...
  assert.equal(inventory.summary.packagedSchemaCount, 140);
  assert.equal(
...
test("schema portfolio covers every Claw and active constraint family", async () => {
  ...
  assert.equal(run.coverage.status, "passed");
  assert.equal(run.coverage.clawCount, 131);
  assert.equal(run.coverage.counts["control-failed"], 0);
  assert.equal(run.coverage.counts.survived, 0);
  assert.equal(run.coverage.counts["unsupported-oracle"], 0);
  assert.equal(run.coverage.counts["oracle-error"], 0);
  assert.equal(run.coverage.safety.caseCount, 786);
  assert.equal(run.coverage.safety.blockingCount, 0);
  assert.equal(run.coverage.schema.clawCount, 131);
...
test("semantic portfolio covers every registered owner-defined validator", async () => {
  ...
  assert.equal(run.coverage.safety.caseCount, 786);
  assert.equal(run.coverage.safety.blockingCount, 0);
  assert.equal(run.coverage.semantics.applicableClawCount, 131);
  assert.equal(run.coverage.semantics.caseCount, 361);
  assert.equal(run.coverage.semantics.killedCount, 361);
  assert.equal(run.coverage.semantics.findingCodeCount, 488);
  assert.equal(Object.keys(run.coverage.semantics.perClaw).length, 131);
...
test("lifecycle portfolio classifies faults and removes every capability adapter", async () => {
  ...
  assert.equal(run.coverage.safety.caseCount, 786);
  assert.equal(run.coverage.safety.blockingCount, 0);
  assert.equal(run.coverage.lifecycle.clawCount, 131);
  assert.equal(run.coverage.lifecycle.caseCount, 655);
  assert.equal(run.coverage.lifecycle.killedCount, 655);
  assert.equal(
