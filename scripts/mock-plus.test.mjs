...
test("inventory derives the current portfolio oracle surface", async () => {
  ...
  assert.equal(inventory.summary.clawCount, 111);
  assert.equal(
  ...
  assert.equal(inventory.summary.packagedSchemaCount, 111);
  assert.equal(
...
test("schema portfolio covers every Claw and active constraint family", async () => {
  ...
  assert.equal(run.coverage.status, "passed");
  assert.equal(run.coverage.clawCount, 111);
  assert.equal(run.coverage.counts["control-failed"], 0);
  assert.equal(run.coverage.counts.survived, 0);
  assert.equal(run.coverage.counts["unsupported-oracle"], 0);
  assert.equal(run.coverage.counts["oracle-error"], 0);
  assert.equal(run.coverage.safety.caseCount, 666);
  assert.equal(run.coverage.safety.blockingCount, 0);
  assert.equal(run.coverage.schema.clawCount, 111);
...
test("lifecycle portfolio classifies faults and removes every capability adapter", async () => {
  ...
  assert.equal(run.coverage.safety.caseCount, 666);
  assert.equal(run.coverage.safety.blockingCount, 0);
  assert.equal(run.coverage.lifecycle.clawCount, 111);
  assert.equal(run.coverage.lifecycle.caseCount, 555);
  assert.equal(run.coverage.lifecycle.killedCount, 555);
  assert.equal(
  ...
  assert.equal(run.coverage.capabilities.applicableClawCount, 51);
  assert.equal(run.coverage.capabilities.caseCount, 74);
  assert.equal(run.coverage.capabilities.killedCount, 74);
