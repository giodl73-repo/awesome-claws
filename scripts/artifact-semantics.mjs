...
function changeDeliveryRecordFindings(value) {
  ...
  const prohibitedNarrative =
    /\b(?:force[- ]?push(?:ed|es|ing)?|rewr(?:ote|ite|ites|iting) (?:the )?(?:git |commit )?history|amend(?:ed|ing)? (?:the |a )?(?:existing |previous )?commit|squash(?:ed|ing)? (?:and )?(?:force|rewr))|\b(?:revert(?:ed|s|ing)?|discard(?:ed|s|ing)?|reformat(?:ted|s|ting)?|stash(?:ed|es|ing)?|overwr(?:ote|ite|ites|iting)) (?:the |their )?(?:user|user's|unrelated|pre-existing|preexisting|uncommitted|someone else's|other people's) \w+|\b(?:skip(?:ped|s|ping)?|disabl(?:e|ed|es|ing)|delet(?:e|ed|es|ing)|remov(?:e|ed|es|ing)|weaken(?:ed|s|ing)?|loosen(?:ed|s|ing)?|comment(?:ed|s|ing)? out) (?:the |a |this )?(?:failing |broken |red )?(?:test|assertion|check|spec|suite)|\b(?:assum(?:e|ed|es|ing)|presum(?:e|ed|es|ing)|report(?:ed|s|ing)?) (?:that )?(?:the )?(?:test|check|build|suite)s? (?:pass|passed|would pass|as passing)|\b(?:fabricat(?:e|ed|es|ing)|invent(?:ed|s|ing)?|synthesi[sz](?:e|ed|es|ing)|made up|mad(?:e|ing) up) (?:a |the |an )?(?:test result|verification|check result|command output|evidence|log|revision)|\b(?:accept(?:ed|s|ing)?|approv(?:e|ed|es|ing)|sign(?:ed|s|ing)? off|decid(?:e|ed|es|ing)) (?:the |this |that )?(?:residual )?(?:risk|change|finding|delivery)? ?on behalf of|\b(?:clos(?:e|ed|es|ing)|comment(?:ed|s|ing)? on|repl(?:y|ied|ies|ying) to) (?:the |a |this )?(?:issue|ticket|pull request|review thread)\b|\b(?:copi(?:ed|es)|past(?:e|ed|es|ing)|embed(?:ded|s|ding)?|includ(?:e|ed|es|ing)) (?:the |a |an )?(?:secret|token|api key|credential|password|private key)|\b(?:chang(?:e|ed|es|ing)|modif(?:y|ied|ies|ying)|edit(?:ed|s|ing)?|touch(?:ed|es|ing)?) (?:the |a |any )?protected (?:path|file|director|area)|\b(?:autonomously|automatically|on our own|without asking|without approval) (?:merg(?:e|ed|es|ing)|push(?:ed|es|ing)?|publish(?:ed|es|ing)?|deliver(?:ed|s|ing)?|releas(?:e|ed|es|ing))/giu;
  const prohibitedDependencyNarrative =
    /\b(?:install(?:ed|s|ing)?|upgrad(?:e|ed|es|ing)|bump(?:ed|s|ing)?) (?:a |the |an )?(?:new |additional )?(?:dependenc(?:y|ies)|package|library|lockfile)/giu;
  const prohibitedPublicationNarrative =
    /\b(?:push(?:ed|es|ing)?|publish(?:ed|es|ing)?) (?:\w+ ){0,3}?(?:to|onto) (?:the )?(?:remote|origin|upstream)|\bopen(?:ed|s|ing)? (?:the |a |this )?(?:\w+ ){0,3}?(?:pull request|merge request)\b/giu;
  const prohibitedSharedBranchNarrative =
    /\b(?:push(?:ed|es|ing)?|publish(?:ed|es|ing)?) (?:\w+ ){0,3}?(?:to|onto) (?:the )?(?:main|master|trunk|default branch|shared branch)/giu;
  const prohibitedMergeNarrative =
    /\b(?:merg(?:e|ed|es|ing)|land(?:ed|s|ing)?) (?:the |a |this )?(?:\w+ ){0,3}?(?:pull request|merge request)\b/giu;
  if (
    hasUnnegatedNarrativeMatch(narrativeTexts, prohibitedNarrative) ||
    (!scope.dependencyChangeAuthorized &&
      hasUnnegatedNarrativeMatch(narrativeTexts, prohibitedDependencyNarrative)) ||
    (performed < performedRank["draft-pull-request-opened"] &&
      hasUnnegatedNarrativeMatch(narrativeTexts, prohibitedPublicationNarrative)) ||
    (performed < performedRank.merged &&
      (hasUnnegatedNarrativeMatch(narrativeTexts, prohibitedSharedBranchNarrative) ||
        hasUnnegatedNarrativeMatch(narrativeTexts, prohibitedMergeNarrative)))
  ) {
    findings.push(
      finding(
        "unsafe_narrative_content",
