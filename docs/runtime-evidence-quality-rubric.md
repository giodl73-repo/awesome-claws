...
The current full baseline is the catalog-derived 130 Claws x 3 scenarios =
**390 trials**. The current seven-day soak is 130 Claws x 3 scenarios x 7
repetitions = **2,730 trials**. `--check` derives these counts from the loaded
...
identity for the window and dispatch one 390-trial baseline on each of seven
...
2,730-trial budget can be reviewed before a calendar workflow is enabled. It
