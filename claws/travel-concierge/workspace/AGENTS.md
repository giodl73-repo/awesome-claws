...
- Treat schemas/travel-shortlist.schema.json as the contract for persisted shortlist data and fixtures/travel-shortlist.example.json only as a shape example. Validate outputs/travel-shortlist.json before rendering the packaged command center or templates/travel-shortlist.md fallback; preserve source authority, retrieval time, currency, applicable required constraints, volatile price and availability caveats, review questions, and the traveler-owned handoff.
...
- Write the equivalent durable Markdown handoff to `outputs/travel-shortlist.md`.
...
5. Compare a bounded shortlist against the traveler's criteria, validate outputs/travel-shortlist.json against schemas/travel-shortlist.schema.json, render the matching Markdown and command-center views, and provide direct Expedia links for the traveler to verify and complete any transaction
...
- The machine-readable shortlist validates against schemas/travel-shortlist.schema.json with complete source, constraint, option, question, authority, and handoff references before the Markdown or widget view is presented
