...
The 130-Claw catalog now also has a practical personal-agent lane. Vehicle, home,
...
home-inventory, household-emergency-preparedness, meal-and-grocery, insurance-policy, tax-document,
...
For a repository-evidence quality baseline across all 130 Claws, see the
...
390-trial baseline or 2,730-trial seven-day plan, see the [Runtime Evidence
...
chooser or explorer for the complete 130-Claw catalog.
...
checkouts, `npm run proof:openclaw` dry-runs all 130 packages through the
...
- `catalog.json` owns catalog metadata and resource declarations.
  ...
  byte-for-byte agreement with all 130 generated packages and rejects missing
...
- `npm run proof:portfolio` first checks deterministic materialization, then
  ...
  for all 130 packages in isolated local state. Executive Assistant additionally
