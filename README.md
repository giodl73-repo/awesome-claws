...
The 129-Claw catalog now also has a practical personal-agent lane. Vehicle, home,
...
For a repository-evidence quality baseline across all 129 Claws, see the
...
387-trial baseline or 2,709-trial seven-day plan, see the [Runtime Evidence
...
chooser or explorer for the complete 129-Claw catalog.
...
checkouts, `npm run proof:openclaw` dry-runs all 129 packages through the
...
- `catalog.json` owns catalog metadata and resource declarations.
  ...
  byte-for-byte agreement with all 129 generated packages and rejects missing
...
- `npm run proof:portfolio` first checks deterministic materialization, then
  ...
  for all 129 packages in isolated local state. Executive Assistant additionally
