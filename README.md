...
The 125-Claw catalog now also has a practical personal-agent lane. Vehicle, home,
...
For a repository-evidence quality baseline across all 125 Claws, see the
...
375-trial baseline or 2,625-trial seven-day plan, see the [Runtime Evidence
...
chooser or explorer for the complete 125-Claw catalog.
...
checkouts, `npm run proof:openclaw` dry-runs all 125 packages through the
...
- `catalog.json` owns catalog metadata and resource declarations.
  ...
  byte-for-byte agreement with all 125 generated packages and rejects missing
...
- `npm run proof:portfolio` first checks deterministic materialization, then
  ...
  for all 125 packages in isolated local state. Executive Assistant additionally
