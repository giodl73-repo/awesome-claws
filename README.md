...
The 119-Claw catalog now also has a practical personal-agent lane. Vehicle, home,
...
purchase-research, household-budget, life-timeline, child-activity, neighborhood-operations, wardrobe, document-renewal, and medical-appointment-prep starters cover everyday bounded
...
For a repository-evidence quality baseline across all 119 Claws, see the
...
357-trial baseline or 2,499-trial seven-day plan, see the [Runtime Evidence
...
chooser or explorer for the complete 119-Claw catalog.
...
checkouts, `npm run proof:openclaw` dry-runs all 119 packages through the
...
- `catalog.json` owns catalog metadata and resource declarations.
  ...
  byte-for-byte agreement with all 119 generated packages and rejects missing
...
- `npm run proof:portfolio` first checks deterministic materialization, then
  ...
  for all 119 packages in isolated local state. Executive Assistant additionally
...
- `npm run validate:artifact -- <claw-id> <artifact.json> [--as-of <RFC3339>] [--trust-keys <keys.json>]`
  ...
  governance artifacts, `--trust-keys` supplies an external JSON array of
  `{ partnerId, partnerName, signingKeyId, publicKey }` trust anchors. Partner
  Business Manager validation requires `--as-of`.
