...
The 126-Claw catalog now also has a practical personal-agent lane. Vehicle, home,
...
For a repository-evidence quality baseline across all 126 Claws, see the
...
378-trial baseline or 2,646-trial seven-day plan, see the [Runtime Evidence
...
chooser or explorer for the complete 126-Claw catalog.
...
checkouts, `npm run proof:openclaw` dry-runs all 126 packages through the
...
- `catalog.json` owns catalog metadata and resource declarations.
  ...
  byte-for-byte agreement with all 126 generated packages and rejects missing
...
- `npm run proof:portfolio` first checks deterministic materialization, then
  ...
  for all 126 packages in isolated local state. Executive Assistant additionally
