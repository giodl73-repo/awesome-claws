...
The 82-Claw catalog now also has a practical personal-agent lane. Vehicle, home,
...
purchase-research, household-budget, and life-timeline starters cover everyday bounded
...
chooser or explorer for the complete 82-Claw catalog.
...
checkouts, `npm run proof:openclaw` dry-runs all 82 packages through the
...
- `catalog.json` owns catalog metadata and resource declarations.
  ...
  byte-for-byte agreement with all 82 generated packages and rejects missing
...
- `npm run proof:portfolio` first checks deterministic materialization, then
  ...
  for all 82 packages in isolated local state. Executive Assistant additionally
