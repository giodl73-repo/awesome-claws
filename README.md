...
The 81-Claw catalog now also has a practical personal-agent lane. Vehicle, home,
...
purchase-research, and household-budget starters cover everyday bounded
...
chooser or explorer for the complete 81-Claw catalog.
...
checkouts, `npm run proof:openclaw` dry-runs all 81 packages through the
...
- `catalog.json` owns catalog metadata and resource declarations.
  ...
  byte-for-byte agreement with all 81 generated packages and rejects missing
...
- `npm run proof:portfolio` first checks deterministic materialization, then
  ...
  for all 81 packages in isolated local state. Executive Assistant additionally
