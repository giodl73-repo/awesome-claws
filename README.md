...
The 87-Claw catalog now also has a practical personal-agent lane. Vehicle, home,
...
chooser or explorer for the complete 87-Claw catalog.
...
checkouts, `npm run proof:openclaw` dry-runs all 87 packages through the
...
- `catalog.json` owns catalog metadata and resource declarations.
  ...
  byte-for-byte agreement with all 87 generated packages and rejects missing
...
- `npm run proof:portfolio` first checks deterministic materialization, then
  ...
  for all 87 packages in isolated local state. Executive Assistant additionally
