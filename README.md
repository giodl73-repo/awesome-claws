...
The 79-Claw catalog now also has a practical personal-agent lane. Vehicle, home,
...
Steward, Work Chief of Staff, and Care Circle Coordinator show the manager-Claw
pattern: they coordinate specialist-Claw artifacts, conflicts, and accountable
decisions without taking over the people or functions that own the work.
...
chooser or explorer for the complete 79-Claw catalog.
...
checkouts, `npm run proof:openclaw` dry-runs all 79 packages through the
...
- `catalog.json` owns catalog metadata and resource declarations.
  ...
  byte-for-byte agreement with all 79 generated packages and rejects missing
...
- `npm run proof:portfolio` first checks deterministic materialization, then
  ...
  for all 79 packages in isolated local state. Executive Assistant additionally
