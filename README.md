...
The 128-Claw catalog now also has a practical personal-agent lane. Vehicle, home,
...
For a repository-evidence quality baseline across all 128 Claws, see the
...
384-trial baseline or 2,688-trial seven-day plan, see the [Runtime Evidence
...
chooser or explorer for the complete 128-Claw catalog.
...
checkouts, `npm run proof:openclaw` dry-runs all 128 packages through the
...
- `catalog.json` owns catalog metadata and resource declarations.
  ...
  byte-for-byte agreement with all 128 generated packages and rejects missing
...
- `npm run proof:portfolio` first checks deterministic materialization, then
  ...
  for all 128 packages in isolated local state. Executive Assistant additionally
