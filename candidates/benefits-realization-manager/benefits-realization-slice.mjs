...
const schema = JSON.parse(
  readFileSync(
    new URL("./schemas/benefits-realization-slice.schema.json", import.meta.url),
    "utf8",
  ),
);
...
function schemaFailureResult(input, schemaFindings) {
  ...
  return {
    schemaVersion: "awesomeClaws.benefitsRealizationSliceResult.v2",
    ledgerId:
...
export function evaluateBenefitsRealizationSlice(input, options = {}) {
  ...
  if (periodStart > periodEnd || periodEnd > cutoff) {
    findings.push(
      finding(
        "invalid-caller-window",
        "request",
        "The caller must supply an ordered period and a cutoff at or after that period; validation never substitutes wall-clock time.",
      ),
    );
  }
...
async function main() {
  ...
  if (
    ...
    throw new Error(
      "Usage: node benefits-realization-slice.mjs evaluate <input> <output> --trust-store <trust.json> --source-bundle <sources.json> [--proof <proof.md>]",
    );
  }
  ...
  const sourceBundleStats = sourceBundlePath
    ? await stat(resolve(sourceBundlePath))
    : null;
  const [trustStore, sourceBundle] = await Promise.all([
    trustStorePath
      ? readFileAsync(resolve(trustStorePath), "utf8").then(JSON.parse)
      : undefined,
    sourceBundlePath &&
    sourceBundleStats.size <=
      BENEFITS_REALIZATION_LIMITS.maxSourceBundleFileBytes
      ? readFileAsync(resolve(sourceBundlePath), "utf8").then(JSON.parse)
      : undefined,
  ]);
  const result = evaluateBenefitsRealizationSlice(input, {
    ...
    sourceBundleByteLength: sourceBundleStats?.size,
  });
  await writeFileAsync(resolve(outputPath), `${JSON.stringify(result, null, 2)}\n`);
  if (proofPath) {
    await writeFileAsync(
